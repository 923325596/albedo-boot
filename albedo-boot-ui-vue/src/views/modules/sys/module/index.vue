  ...
  <div class="app-container calendar-list-container">
      ...
      <el-col :span="19">
        ...
        <el-table :data="list" v-loading="listLoading" element-loading-text="加载中..." border fit highlight-current-row style="width: 99%">
          ...
          <el-table-column align="center" label="操作">
            <template slot-scope="scope">
              <el-button v-if="sys_module_edit" icon="icon-edit" title="编辑" type="text" @click="handleEdit(scope.row)">
              </el-button>
              <el-button v-if="sys_module_lock" :icon="scope.row.status=='正常' ? 'icon-lock' : 'icon-unlock'" :title="scope.row.status=='正常' ? '锁定' : '解锁'" type="text" @click="handleLock(scope.row)">
              </el-button>
              <el-button v-if="sys_module_delete" icon="icon-delete" title="删除" type="text" @click="handleDelete(scope.row)">
              </el-button>
            </template>
          </el-table-column>
    ...
    <el-dialog title="查看图标" :visible.sync="dialogIconVisible">
      <ul class="icon_lists clear">
        ...
        <li>
          <i class="icon iconfont icon-file-excel-fill"></i>
          <div class="name">file-excel-fill</div>
          <div class="fontclass">.icon-file-excel-fill</div>
        </li>
...
<script>
  import {fetchModuleTree, findModule, saveModule, removeModule, pageModule, lockModule} from "./service";
